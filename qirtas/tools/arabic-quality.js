...
function arabicWordRatio(text) {
  const toks = arTokens(String(text || '').slice(0, 20000));
  ...
  for (const t of toks) {
    ...
    if (ar / letters.length > 0.7 && letters.length >= 2 && letters.length <= 18) ok++;
  }
...
function textOf(id) {
  ...
  const p = path.join(DIST, 'books', id + '.json');
  let txt = '';
  try {
...
const report = { total: rows.length, junkTitles: [], placeholderAuthors: [], lowQualityText: [], latinTitlesInArabic: [] };
...
for (const r of rows) {
  ...
  if (r.language === 'ar') {
    const ratio = arabicWordRatio(textOf(r.id));
    if (ratio < 0.55) report.lowQualityText.push({ id: r.id, ratio: +ratio.toFixed(3), words: r.words, title: title.slice(0, 50), source: r.source_key });
  }
}
